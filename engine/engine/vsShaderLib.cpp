...
void VSShaderLib::setVertexAttribName(GLuint index, GLuint at, std::string name) {
	glBindAttribLocation(index, at, name.c_str());
}
...
VSShaderLib::setUniform(std::string name, void *value) {
	...
	case GL_FLOAT_MAT4:
		glUseProgram(pProgram);
		glUniformMatrix4fv(u.location, 1, GL_FALSE, (const GLfloat *)value);
